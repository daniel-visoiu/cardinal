...
export function highlightChapter(evt: PskScrollEvent): void {
    let self = this;

    const scrollSectionElement: HTMLElement =
        evt.parentEventData && (evt.parentEventData as HTMLElement);
    ...
    const pageHeight: number = scrollSectionElement.offsetHeight;
...
function _getVerticalOffset(
    ...
    if (previousEntry) {
        ...
        if (verticalOffset > vOffset || verticalOffset + height > vOffset + vHeight) {
